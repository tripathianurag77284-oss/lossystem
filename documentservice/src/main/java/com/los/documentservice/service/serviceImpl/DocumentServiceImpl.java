...
import com.los.documentservice.exception.ResourceNotFoundException;
...
public class DocumentServiceImpl implements DocumentService {
    ...
    public List<DocumentResponse> getAllDocuments() {
        return documentRepository.findAllByIsDeletedFalse().stream().map(this::mapToResponse).toList();
    }

    @Override
    public DocumentResponse getDocumentById(Long documentId) {
        return mapToResponse(findDocument(documentId));
    }
    ...
    public DocumentResponse createDocument(DocumentRequest request, MultipartFile file) {
        ...
        try {
            ...
            document.setVerificationMethod(request.getVerificationMethod());
            ...
            document.setValidatedFlag(Boolean.TRUE.equals(request.getValidatedFlag()));
            ...
            document.setVerifiedAt(Boolean.TRUE.equals(request.getVerifiedFlag()) ? now : null);
            document.setValidatedAt(Boolean.TRUE.equals(request.getValidatedFlag()) ? now : null);
            document.setCreatedBy(request.getCreatedBy());
            document.setModifiedBy(request.getModifiedBy());
            document.setVerifiedBy(Boolean.TRUE.equals(request.getVerifiedFlag()) ? request.getVerifiedBy() : null);
            document.setValidatedBy(Boolean.TRUE.equals(request.getValidatedFlag()) ? request.getValidatedBy() : null);
    ...
    public DocumentResponse updateDocument(Long documentId, DocumentRequest request, MultipartFile file) {
        ...
        document.setModifiedBy(request.getModifiedBy());
        updateVerificationAudit(document, request);
        updateValidationAudit(document, request);
    ...
    private void updateVerificationAudit(Document document, DocumentRequest request) {
        if (Boolean.TRUE.equals(request.getVerifiedFlag())) {
            if (document.getVerifiedAt() == null) document.setVerifiedAt(LocalDateTime.now());
    ...
    private void updateValidationAudit(Document document, DocumentRequest request) {
        if (Boolean.TRUE.equals(request.getValidatedFlag())) {
            if (document.getValidatedAt() == null) document.setValidatedAt(LocalDateTime.now());
    ...
    private DocumentResponse mapToResponse(Document document) {
        return new DocumentResponse(
                document.getDocumentId(), document.getApplicantId(), document.getDocumentTypeId(),
                document.getUploadStage(), document.getFileUrl(), document.getVerifiedFlag(),
                document.getVerificationRemark(), document.getVerificationStatus(), document.getVerificationMode(),
                document.getValidatedFlag(), document.getValidationRemark(), document.getValidationStatus(),
                document.getValidationMode(), document.getIsActive(), document.getCreatedAt(),
                document.getModifiedAt(), document.getVerifiedAt(), document.getValidatedAt(),
                document.getCreatedBy(), document.getModifiedBy(), document.getVerifiedBy(),
                document.getValidatedBy(), document.getIsDeleted()
        );
    }
}
